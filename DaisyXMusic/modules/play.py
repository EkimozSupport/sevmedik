...
async def settings(client, message):
    ...
    if stats:
        if playing:
            await message.reply(stats, reply_markup=r_ply("paauusse")) 
        else:
            await message.reply(stats, reply_markup=r_ply("sikişşş1"))
    else:
...
@Client.on_callback_query(filters.regex(pattern=r"^(playlisttt3)$"))
async def p_cb(b, cb):
    global que
...
@Client.on_callback_query(
    filters.regex(pattern=r"^(sikişşş1|paauusse|s2|l3|puse|r4|menu|cls)$")
)
@cb_admin_check
async def m_cb(b, cb):
    global que
    ...
    if type_ == "paauusse":
        if (chet_id not in callsmusic.active_chats) or (
            ...
            callsmusic.paauusse(chet_id)
            await cb.answer("Müzik Duraklatıldı!")
            await cb.message.edit(
                updated_stats(m_chat, qeue), reply_markup=r_ply("skişşş1")
            )
        ...
        if (chet_id not in callsmusic.active_chats) or (
            ...
            await cb.message.edit(
                updated_stats(m_chat, qeue), reply_markup=r_ply("paauusse")
            )

    elif type_ == "playlisttt3":
        queue = que.get(cb.message.chat.id)
        if not queue:
            await cb.message.edit("Player is idle")
        temp = []
...
@Client.on_message(command("sikişşş1") & other_filters)
async def sikişşş1(_, message: Message):
    global que
    global useer
    if message.chat.id in DISABLED_GROUPS:
        return    
